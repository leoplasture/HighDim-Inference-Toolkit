...
class TransLasso:
    ...
    def fit(
        ...
        # Store per-dataset fits for a simple informative-set heuristic.
    ...
    def get_informative_set(self, threshold: float) -> list[int]:
        """Identify informative auxiliary datasets using a simple sparsity proxy.

        We approximate the contrast sparsity by comparing auxiliary-only Lasso fits against
        the target-only Lasso fit: diff_k = w_aux^(k) - w_target.

        Args:
           threshold:
              - If 0 < threshold < 1: maximum fraction of coordinates allowed to differ.
              - Else: maximum number of coordinates allowed to differ.

        Returns:
           A list of indices into the auxiliary dataset lists.
        """
